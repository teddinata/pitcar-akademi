<template>
  <div class="min-h-screen clay-surface p-4 md:p-6">
    ...
    <!-- Course selector -->
    <div class="clay-card p-4 mb-5 flex flex-wrap items-center gap-3">
      <label class="text-sm font-semibold text-gray-600 shrink-0">Kursus:</label>
      <select
        v-model="selectedCourseId"
        @change="onBuilderCourseChange"
        class="text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-blue-300 min-w-56 flex-1 max-w-md"
      >
    ...
    <!-- Belum pilih kursus -->
    <div v-else-if="!selectedCourseId" class="bg-white rounded-xl p-12 text-center border border-gray-100 shadow-sm">
      <PuzzlePieceIcon class="w-12 h-12 text-gray-300 mx-auto mb-3" />
      <p class="text-gray-500 font-medium">Pilih kursus untuk mengelola modulnya.</p>
    </div>

    <!-- COURSE BUILDER -->
    <div v-else class="space-y-4">
      <!-- Sections -->
      <div v-for="(sec, si) in grouped.sections" :key="sec.id" class="clay-card overflow-hidden">
        <!-- Section header -->
        <div class="flex items-center gap-2 px-4 py-3 bg-gray-50/80 border-b border-gray-100">
          <button @click="toggleCollapse(sec.id)" class="w-5 shrink-0 text-gray-400 hover:text-gray-700 text-sm leading-none">
            {{ isCollapsed(sec.id) ? '▸' : '▾' }}
          </button>
          <div class="flex-1 min-w-0 cursor-pointer" @click="toggleCollapse(sec.id)">
            <p class="font-bold text-gray-800 text-sm truncate">{{ sec.name }}</p>
            <p class="text-[11px] text-gray-400">{{ sec.modules.length }} modul</p>
          </div>
          <div class="flex items-center gap-1 shrink-0">
            <button @click="moveBuilderSection(si, -1)" :disabled="si === 0" class="px-1.5 text-gray-400 hover:text-gray-700 disabled:opacity-30 leading-none" title="Naik">▲</button>
            <button @click="moveBuilderSection(si, 1)" :disabled="si === grouped.sections.length - 1" class="px-1.5 text-gray-400 hover:text-gray-700 disabled:opacity-30 leading-none" title="Turun">▼</button>
            <button @click="openSectionManager" class="ml-1 px-2 py-1 text-[11px] border border-gray-200 text-gray-500 rounded-lg hover:bg-white transition-colors" title="Kelola section">
        ...
        <!-- Modules in section -->
        <div v-if="!isCollapsed(sec.id)">
          <div v-if="!sec.modules.length" class="px-4 py-3 text-xs text-gray-400 text-center">Belum ada modul di section ini.</div>
          <div
            v-for="(m, mi) in sec.modules"
            :key="m.id"
            class="flex items-center gap-3 px-4 py-2.5 border-b border-gray-50 hover:bg-gray-50/70 transition-colors"
          >
            <span class="text-lg shrink-0">{{ contentTypeIcon(m.content_type) }}</span>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-gray-800 truncate">{{ m.name }}</p>
              <p class="text-[11px] text-gray-400 flex items-center gap-1.5 flex-wrap">
                <span class="px-1.5 py-0.5 rounded-full font-medium" :class="contentTypeClass(m.content_type)">{{ contentTypeLabel(m.content_type) }}</span>
                <template v-if="m.content_type === 'assessment'">
                  <span v-if="m.quiz_bank_name" class="px-1.5 py-0.5 rounded-full bg-purple-50 text-purple-700 font-medium">🧩 {{ m.quiz_bank_name }}</span>
                  <span v-else class="px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700 font-medium">Quiz belum diatur</span>
                </template>
                <span>· {{ m.duration_minutes }} mnt</span>
              </p>
            </div>
            <div class="flex items-center gap-1 shrink-0">
              <button @click="moveBuilderModule(sec.modules, mi, -1)" :disabled="mi === 0" class="px-1 text-gray-300 hover:text-gray-600 disabled:opacity-30 leading-none" title="Naik">▲</button>
              <button @click="moveBuilderModule(sec.modules, mi, 1)" :disabled="mi === sec.modules.length - 1" class="px-1 text-gray-300 hover:text-gray-600 disabled:opacity-30 leading-none" title="Turun">▼</button>
              <button
                v-if="m.content_type === 'assessment' && m.quiz_bank_id"
                @click="openQuestionManager(m)"
                class="ml-1 px-2 py-1 text-xs border border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 transition-colors"
                title="Kelola Soal"
              ><ListBulletIcon class="w-3.5 h-3.5" /></button>
              <button v-else-if="m.content_type !== 'assessment'" @click="openUploadModal(m)" class="ml-1 px-2 py-1 text-xs border border-blue-200 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors" title="Atur Konten">
                <ArrowUpTrayIcon class="w-3.5 h-3.5" />
              </button>
              <button @click="openEditModal(m)" class="px-2 py-1 text-xs border border-gray-200 text-gray-500 rounded-lg hover:bg-gray-50 transition-colors" title="Edit"><PencilIcon class="w-3.5 h-3.5" /></button>
              <button @click="deleteModule(m)" class="px-2 py-1 text-xs border border-red-100 text-red-400 rounded-lg hover:bg-red-50 transition-colors" title="Hapus"><TrashIcon class="w-3.5 h-3.5" /></button>
            </div>
          </div>
          <!-- Add module to this section -->
          <button
            @click="openCreateForSection(selectedCourseId, sec.id)"
            class="w-full px-4 py-2.5 text-xs font-semibold text-blue-600 hover:bg-blue-50 transition-colors flex items-center gap-1.5"
          >
            <PlusIcon class="w-3.5 h-3.5" /> Tambah modul ke "{{ sec.name }}"
          </button>
        </div>
      </div>

      <!-- Root (tanpa section) -->
      <div v-if="grouped.root.length" class="clay-card overflow-hidden">
        <div class="flex items-center gap-2 px-4 py-3 bg-gray-50/80 border-b border-gray-100">
          <button @click="toggleCollapse('root')" class="w-5 shrink-0 text-gray-400 hover:text-gray-700 text-sm leading-none">
            {{ isCollapsed('root') ? '▸' : '▾' }}
          </button>
          <div class="flex-1 min-w-0 cursor-pointer" @click="toggleCollapse('root')">
            <p class="font-bold text-gray-500 text-sm">Tanpa Section</p>
            <p class="text-[11px] text-gray-400">{{ grouped.root.length }} modul</p>
          </div>
        </div>
        <div v-if="!isCollapsed('root')">
          <div
            v-for="(m, mi) in grouped.root"
            :key="m.id"
            class="flex items-center gap-3 px-4 py-2.5 border-b border-gray-50 hover:bg-gray-50/70 transition-colors"
          >
            <span class="text-lg shrink-0">{{ contentTypeIcon(m.content_type) }}</span>
            ...
            <div class="flex items-center gap-1 shrink-0">
              <button @click="moveBuilderModule(grouped.root, mi, -1)" :disabled="mi === 0" class="px-1 text-gray-300 hover:text-gray-600 disabled:opacity-30 leading-none" title="Naik">▲</button>
              <button @click="moveBuilderModule(grouped.root, mi, 1)" :disabled="mi === grouped.root.length - 1" class="px-1 text-gray-300 hover:text-gray-600 disabled:opacity-30 leading-none" title="Turun">▼</button>
              <button
                v-if="m.content_type === 'assessment' && m.quiz_bank_id"
                @click="openQuestionManager(m)"
                class="ml-1 px-2 py-1 text-xs border border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 transition-colors"
      ...
      <div class="flex flex-wrap gap-2">
        <button
          @click="openCreateForSection(selectedCourseId, null)"
          class="flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm"
        >
          <PlusIcon class="w-4 h-4" /> Tambah Modul
        </button>
        <button
          @click="openSectionManager"
          class="flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-colors shadow-sm"
        >
          <RectangleStackIcon class="w-4 h-4" /> Tambah / Kelola Section
        </button>
      </div>
    </div>
...
// ── COURSE BUILDER ──────────────────────────────────────────────────────────
// Struktur hierarkis: section (urut) → modul bersarang, + grup "tanpa section".
const grouped = computed(() => {
  ...
  const root = modules.value
    .filter(m => !m.section_id)
    .sort((a, b) => (a.sequence || 0) - (b.sequence || 0))
  return { sections: secs, root }
})

function isCollapsed(key) { return !!collapsed[key] }
function toggleCollapse(key) { collapsed[key] = !collapsed[key] }

function onBuilderCourseChange() {
  load()
  // Refresh daftar section pada form modul jika terbuka
  loadFormSections(selectedCourseId.value)
}
...
async function load() {
  ...
  try {
    const [mres, sres] = await Promise.all([
      lmsApi.moduleSearch({ course_id: selectedCourseId.value, limit: 500 }),
      lmsApi.sectionSearch({ course_id: selectedCourseId.value }),
    ])
    modules.value = mres?.modules ?? []
    sections.value = sres?.sections ?? []
  } catch (e) {
...
// Buat modul dengan course & section otomatis terisi (kontekstual)
function openCreateForSection(courseId, sectionId) {
  editTarget.value = null
  Object.assign(form, emptyForm())
  form.course_id = courseId
  form.section_id = sectionId || null
  // urutan default = setelah modul terakhir di section tsb
  const target = sectionId
    ? (grouped.value.sections.find(s => s.id === sectionId)?.modules || [])
    : grouped.value.root
  form.sequence = (target.length + 1) * 10
  modalError.value = ''
  showModal.value = true
  loadCourses()
  loadFormSections(courseId)
  loadCourseQuizBanks()
}

async function moveBuilderSection(index, dir) {
  const secs = grouped.value.sections
  ...
  try {
    ...
    showToast(e.message || 'Gagal mengubah urutan section')
  }
}

async function moveBuilderModule(list, index, dir) {
  const target = index + dir
  ...
  try {
    await Promise.all(
      arr.map((m, i) => {
        const seq = (i + 1) * 10
        if (m.sequence === seq) return null
        return lmsApi.moduleUpdate({ module_id: m.id, sequence: seq })
      }).filter(Boolean)
    )
    await load()
  } catch (e) {
    showToast(e.message || 'Gagal mengubah urutan modul')
    await load()
  }
}
...
function openSectionManager() {
  ...
  sm.error = ''
  sm.newName = ''
  sm.courseId = selectedCourseId.value || null
  loadCourses()
...
onMounted(async () => {
  await loadCourses()
  // Default: pilih kursus pertama agar tidak menampilkan halaman kosong
