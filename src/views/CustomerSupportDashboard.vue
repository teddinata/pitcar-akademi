...
const exportAllKPIsToPDF = () => {
  try {
    ...
    form.method = 'POST'
    form.action = '/web/v2/kpi/customer-support/export_pdf'
    form.target = '_blank'
  ...
  try {
    ...
    if (response.data.result.status === 'success') {
      employees.value = response.data.result.data.filter(emp => 
        ['Lead Customer Support', 'Service Advisor', 'Customer Service', 'Valet Parking', 'Admin Part', 'Partman', 'Toolkeeper'].includes(emp.job_title)
      )
    }
  } catch (error) {
