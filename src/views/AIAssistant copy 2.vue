<template>
  <div class="min-h-screen bg-gray-50 flex">
    ...
    <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
              ...
              <form @submit.prevent="sendFromWelcome">
                <div class="relative bg-white border border-gray-300 rounded-lg transition-all focus-within:ring-2 focus-within:ring-blue-300 focus-within:border-blue-500">
                  <textarea 
                    ...
                    placeholder="Try: 'What were my top selling products last month?' or 'How can I improve customer retention?'"
                  ...
                  <div 
                    v-if="welcomeMessage.trim().length > 5 && welcomeQueryMode === 'auto'"
                    class="absolute top-2 right-2 px-2 py-0.5 rounded-full text-xs font-medium"
                    :class="{
                      'bg-blue-100 text-blue-700': detectBusinessContext(welcomeMessage),
                      'bg-purple-100 text-purple-700': !detectBusinessContext(welcomeMessage)
                    }"
                  >
                    <component 
                      :is="detectBusinessContext(welcomeMessage) ? 'ChartBarIcon' : 'ChatBubbleLeftRightIcon'" 
                      class="w-3 h-3 inline mr-0.5" 
                    />
                    {{ detectBusinessContext(welcomeMessage) ? 'Business' : 'General' }}
                  </div>
      ...
      <div v-else class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <!-- Header -->
        <div class="flex items-center p-4 bg-white border-b border-gray-200 flex-shrink-0">
          <button @click="toggleSidebar" class="p-2 mr-3 text-gray-500 hover:text-gray-700 lg:hidden">
            <Bars3Icon class="w-6 h-6" />
          </button>
          <h2 class="text-lg font-semibold text-gray-800 flex-1 truncate">
            {{ currentChat ? currentChat.name : 'AI Business Assistant' }}
          ...
          <div 
            v-if="currentChat && currentChat.isTemporary" 
            class="flex items-center mr-3 bg-yellow-100 border border-yellow-200 rounded-full px-2 py-1 animate-pulse"
          >
            <ClockIcon class="w-3.5 h-3.5 text-yellow-500 mr-1" />
            <span class="text-xs text-yellow-700 font-medium">Unsaved Draft</span>
          </div>
          ...
          <div class="flex">
            <button 
              ...
              title="Export"
              :disabled="currentChat && currentChat.isTemporary"
              :class="{'opacity-50 cursor-not-allowed': currentChat && currentChat.isTemporary}"
            >
              <ArrowDownTrayIcon class="w-5 h-5" />
            </button>
            <button @click="clearChat" class="p-2 text-gray-500 hover:text-gray-700" title="Clear">
        ...
        <div class="flex-1 overflow-y-auto p-5" ref="messagesContainer">
          <div 
            v-if="currentChat && currentChat.isTemporary" 
            class="mb-4 px-4 py-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm flex items-center justify-between"
          >
            <div class="flex items-center">
              <InformationCircleIcon class="w-5 h-5 mr-2 text-yellow-500" />
              <span class="text-yellow-700">
                This is a draft conversation. Send a message to save it.
              </span>
            </div>
            
            <button 
              @click="discardTemporaryChat" 
              class="text-yellow-600 hover:text-yellow-800 text-xs font-medium"
            >
              <XMarkIcon class="w-4 h-4 inline" /> Discard
            </button>
          </div>

          <!-- Temporary Chat Indicator in Sidebar -->
          <span 
          v-if="chat.isTemporary" 
          class="px-1.5 py-0.5 text-[10px] bg-yellow-100 text-yellow-700 rounded-full ml-1"
          >
          Draft
          </span>
...
const welcomeQueryMode = ref('auto');
const tempChat = ref(null);

// Fungsi untuk memeriksa apakah suatu chat bersifat sementara
function isTemporaryChat(chat) {
  // Safe check: return false jika chat undefined atau null
  return chat && chat.isTemporary === true;
}

// Fungsi untuk mengirim pesan dari welcome screen
async function sendFromWelcome() {
  if (!welcomeMessage.value.trim()) return;
  
  // Buat chat sementara (umum atau bisnis berdasarkan mode dan konteks pesan)
  const messageText = welcomeMessage.value.trim();
  let isBusinessContext = welcomeQueryMode.value === 'business';
  
  // Auto-detect bisnis jika mode 'auto'
  if (welcomeQueryMode.value === 'auto') {
    isBusinessContext = detectBusinessContext(messageText);
  }
  
  // Buat chat sementara sesuai konteks
  ...
  // Isi form input dengan pesan dari welcome screen
  newMessage.value = messageText;
  
  // Reset input welcome
  welcomeMessage.value = '';
  
  // Sedikit delay untuk memastikan UI telah dirender
  setTimeout(() => {
...
// Fungsi untuk deteksi konteks bisnis sederhana
function detectBusinessContext(message) {
  ...
  const businessKeywords = [
    ...
    'analysis', 'analisis', 'chart', 'grafik', 'dashboard'
  ];
...
async function selectChat(chatId) {
  ...
  // Pemeriksaan yang aman untuk temporary chat
  if (tempChat.value && currentChat.value && 
      chatId !== currentChat.value.id && 
      isTemporaryChat(currentChat.value)) {
    if (!confirm('You have an unsaved chat. Discard it and switch to another chat?')) {
      return;
    }
    // Reset temporary chat
    tempChat.value = null;
  }

  // Jika sama dengan chat yang aktif, tidak perlu reload
  ...
  try {
    ...
    if (result && result.success) {
      tempChat.value = null; // Reset temporary chat
      ...
      showError('Failed to load messages: ' + (result ? result.error : 'Unknown error'));
    }
  } catch (error) {
...
function createNewChat() {
  // Buat chat sementara dengan ID lokal
  const newTempChat = {
    id: 'temp-' + Date.now(),
    ...
    total_messages: 0,
    total_tokens: 0,
    isTemporary: true, // Penanda bahwa ini adalah chat sementara
    category: 'general'
  };
  
  // Set sebagai temporary chat
  tempChat.value = newTempChat;
  
  // Set sebagai chat aktif
  currentChat.value = newTempChat;
  ...
  // Sembunyikan sidebar pada perangkat mobile
  if (window.innerWidth < 768) {
    showSidebar.value = false;
  }
  
  console.log('Created temporary chat:', currentChat.value);
}

// Create a new chat specifically for business analysis
function createNewBusinessChat() {
  // Buat chat sementara dengan ID lokal
  const newTempChat = {
    id: 'temp-' + Date.now(),
  ...
  // Sembunyikan sidebar pada mobile
  if (window.innerWidth < 768) {
    showSidebar.value = false;
  }
  
  console.log('Created temporary business chat:', currentChat.value);
}

async function sendMessage() {
  // Periksa kondisi valid untuk mengirim pesan
  if (!currentChat.value) {
    console.error('No active chat found');
    return;
  }
  
  // Periksa apakah ada pesan untuk dikirim
  const messageContent = dataQueryMode.value ? 
    generateQueryPreview() : 
    newMessage.value.trim();
  
  if (!messageContent && !dataQueryMode.value) {
  ...
  // Simpan sementara dan reset input
  ...
  try {
    // Periksa dengan aman jika ini adalah chat sementara
    let chatId = currentChat.value.id;
    
    // Jika ini adalah chat sementara, buat chat baru di server dulu
    if (isTemporaryChat(currentChat.value)) {
      console.log('Creating new chat on server for temporary chat:', currentChat.value);
      ...
      try {
        const chatResponse = await apiClient.post('/web/ai/chat', {
          ...
          params: {
            ...
            params: {
              ...
              category: currentChat.value.category || 'general'
            }
          },
          id: Math.floor(Math.random() * 1000000)
        });
        ...
        // Pastikan chat berhasil dibuat
        if (chatResponse.data.result && chatResponse.data.result.success) {
          // Update referensi ke chat yang baru dibuat
          const realChatId = chatResponse.data.result.chat_id;
          chatId = realChatId;
          
          // Simpan data chat lama
          const oldChatData = { ...currentChat.value };
          
          // Update dengan ID baru dan hapus flag sementara
          currentChat.value = {
            ...oldChatData,
            id: realChatId,
            isTemporary: false,
          ...
          console.log('Chat created successfully with ID:', realChatId);
          ...
          // Setelah sukses, kosongkan temporary chat
          tempChat.value = null;
        } else {
          // Handle error jika chat gagal dibuat
        ...
        // Tambahkan pesan error ke messages
        messages.value.push({
          id: Date.now(),
        ...
        // Keluar dari fungsi karena gagal membuat chat
        isThinking.value = false;
        return;
      }
    }
    
    // Sekarang kirim pesan ke chat yang ada
    const requestParams = {
      chat_id: chatId,
    ...
    console.log('Message content:', pendingMessage);
    ...
    if (result && result.success) {
      ...
      updateChatListPreview(chatId, pendingMessage);
      
      // Jika ini pesan pertama setelah menciptakan chat, mungkin nama chat telah diperbarui
      if (result.chat_updated && result.chat) {
        console.log('Chat updated with new info:', result.chat);
        updateChatInList(result.chat);
      }
...
function updateChatInList(updatedChat) {
  if (!updatedChat || !updatedChat.id) {
    console.error('Invalid chat data for update');
    return;
  }
  
  const index = chatList.value.findIndex(chat => chat.id === updatedChat.id);
  if (index !== -1) {
    // Update properties yang berubah
    chatList.value[index] = {
      ...chatList.value[index],
      name: updatedChat.name,
      topic: updatedChat.topic || chatList.value[index].topic,
      // update properties lain jika diperlukan
    };
    
    // Update juga referensi currentChat jika ini adalah chat yang aktif
    if (currentChat.value && currentChat.value.id === updatedChat.id) {
      currentChat.value = {
        ...currentChat.value,
        name: updatedChat.name,
        topic: updatedChat.topic || currentChat.value.topic,
      };
    }
  }
}
...
function clearChat() {
  ...
  // Jika temporary chat, cukup reset messages
  if (isTemporaryChat(currentChat.value)) {
    messages.value = [];
    return;
  }
  
  // Jika chat normal, konfirmasi seperti biasa
  if (!confirm('Are you sure you want to clear all messages? This cannot be undone.')) return;
  
  try {
    apiClient.post('/web/ai/chat', {
      jsonrpc: "2.0",
      method: "call",
      params: {
      ...
      if (result && result.success) {
        ...
        showError('Failed to clear chat: ' + (result ? result.error : 'Unknown error'));
      }
    }).catch(error => {
...
function discardTemporaryChat() {
  if (currentChat.value && isTemporaryChat(currentChat.value)) {
    currentChat.value = null;
    tempChat.value = null;
    messages.value = [];
  }
...
function exportChat() {
  ...
  // Jika chat sementara, tampilkan pesan
  if (isTemporaryChat(currentChat.value)) {
    showNotification('Please send a message first to save this chat before exporting');
    return;
  }
...
onMounted(() => {
  ...
  // Focus input pada welcome screen
  nextTick(() => {
...
onMounted(() => {
  window.addEventListener('popstate', (event) => {
    if (currentChat.value && currentChat.value.isTemporary) {
      ...
      if (confirm('Discard current unsaved chat?')) {
